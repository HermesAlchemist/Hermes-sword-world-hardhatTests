...
describe("Sword", function () {
  ...
  describe("Minting", function () {
    it("Should mint a new sword and assign the correct owner", async function () {
      const { sword, otherAccount, owner } = await loadFixture(deploySwordFixture);

      // Mint a new sword
      const mintTx = await sword.connect(otherAccount).mint();
    ...
    it("Should increment token IDs correctly", async function () {
      ...
      expect(await sword.balanceOf(owner.address)).to.equal(3);
    });
  });
});
